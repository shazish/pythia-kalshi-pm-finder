...
```
terminal(command="claude -p 'task' --fallback-model haiku --max-turns 5", workdir="/project", timeout=90)
```
Automatically falls back to the specified model when the default model is overloaded (print mode only).
...
| Flag | Effect |
|------|--------|
| `--output-format <fmt>` | `text` (default), `json` (single result), `stream-json` (newline-delimited) |
...
terminal(command="tmux send-keys -t review 'cd /path/to/repo && claude' Enter")
...
- **`sonnet`** (default) — best balance of speed and quality for most tasks
- **`opus`** — highest quality, slower, more expensive; best for complex analysis or code review
- **`haiku`** — fastest, cheapest; best for simple tasks, formatting, quick edits
- **`claude-sonnet-4-6`** — pinned alias for reproducibility

`--effort max` + `opus` = maximum reasoning but slow and expensive. Use sparingly.
...
12. **Use `--json-schema`** when you need structured output — implies the `---json-schema` flag.
...
13. **`-p` prompt must be a command argument, NOT a heredoc or stdin** — passing the prompt via heredoc or piped stdin causes Claude to silently enter interactive TUI mode and block on the workspace trust dialog. Always pass the prompt directly in the call: `claude -p "your prompt"`. For long or special-character prompts, write the prompt to a file and pass it as `"$PROMPT"` on the command line (using a shell script wrapper).
...
11. **`-p` prompt must be a direct argument** — NEVER pipe via stdin / heredoc with `-p`; use a shell script wrapper for complex prompts
